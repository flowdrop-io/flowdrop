...
<script lang="ts">
  ...
  // Update local values when props change and manage focus
  $effect(() => {
  ...
  // Check for changes
  $effect(() => {
    hasChanges = JSON.stringify(localConfigValues) !== JSON.stringify(props.configValues);
  });
   ...
   */
  function isTextarea(property: any): boolean {
  ...
  <div class="config-sidebar__content">
      ...
      <div class="config-sidebar__details">
        ...
        <div class="config-sidebar__detail-grid">
          ...
          <div class="config-sidebar__detail-item">
            ...
            <span class="config-sidebar__detail-value config-sidebar__detail-value--badge">{props.nodeDetails.category}</span>
          </div>
        ...
        {#if props.nodeDetails.inputs && props.nodeDetails.inputs.length > 0}
          <div class="config-sidebar__detail-section">
            ...
            <div class="config-sidebar__ports">
              {#each props.nodeDetails.inputs as input}
        ...
        {#if props.nodeDetails.outputs && props.nodeDetails.outputs.length > 0}
          <div class="config-sidebar__detail-section">
            ...
            <div class="config-sidebar__ports">
              {#each props.nodeDetails.outputs as output}
        ...
        {#if props.nodeDetails.tags && props.nodeDetails.tags.length > 0}
          <div class="config-sidebar__detail-section">
            ...
            <div class="config-sidebar__tags">
              {#each props.nodeDetails.tags as tag}
    ...
    {#if props.configSchema?.properties}
      <div class="config-sidebar__config-section">
        ...
        <form class="config-sidebar__form" onsubmit={(e) => { e.preventDefault(); handleSave(); }}>
        {#each Object.entries(props.configSchema.properties) as [key, property]}
          <div class="config-sidebar__field">
            ...
            {#if property.enum}
              ...
              <select
                ...
                onchange={(e) => handleInputChange(key, (e.target as HTMLSelectElement).value, property.type)}
              >
            ...
            {:else if property.type === "boolean"}
              ...
              <div class="config-sidebar__checkbox-wrapper">
                <input
                  ...
                  onchange={(e) => handleInputChange(key, (e.target as HTMLInputElement).checked, property.type)}
                />
            ...
            {:else if isTextarea(property)}
              ...
              <textarea
                ...
                value={getDisplayValue(localConfigValues[key] ?? property.default ?? "", property.type)}
                oninput={(e) => handleInputChange(key, (e.target as HTMLTextAreaElement).value, property.type)}
                rows="4"
              ></textarea>
            {:else}
              <!-- Regular input -->
              <input
                ...
                oninput={(e) => handleInputChange(key, (e.target as HTMLInputElement).value, property.type)}
              />
            {/if}
          </div>
        {/each}
