 ...
 */
function validateWorkflowData(data: any): { isValid: boolean; error?: string } {
  ...
  if (data.tags && data.tags.some((tag: any) => typeof tag !== "string" || tag.length > 50)) {
  ...
  try {
    ...
    const workflowData: Omit<Workflow, "id"> = {
      ...
      metadata: {
        ...
        tags: Array.isArray(body.tags) ? body.tags.filter((tag: string) => tag.trim().length > 0) : []
      }
    };
