...
<script lang="ts">
  ...
  async function testWorkflowApi(): Promise<void> {
    try {
      ...
      testResults.push(`✗ Workflow API error: ${err instanceof Error ? err.message : "Unknown error"}`);
      error = err instanceof Error ? err.message : "Unknown error";
    }
  }
  ...
  async function runAllTests(): Promise<void> {
    ...
    try {
      ...
      testResults.push(`💥 Test suite failed: ${err instanceof Error ? err.message : "Unknown error"}`);
    } finally {
      loading = false;
    }
  }
                  ...
                  <div class="card-body p-4">
                    <div class="flex items-center justify-between">
                      ...
                      <div class="flex gap-1">
                        {#each workflow.metadata?.tags || [] as tag}
                          <div class="badge badge-outline badge-sm">{tag}</div>
                        {/each}
                      </div>
                    </div>
