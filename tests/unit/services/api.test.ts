	...
	describe("nodeApi", () => {
		describe("getNodes", () => {
			it("should fetch all nodes", async () => {
				...
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchResponse(mockData.data))
				);
			...
			it("should handle fetch errors", async () => {
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Server error", 500))
				);
		...
		describe("getNode", () => {
			...
			it("should handle node not found", async () => {
				...
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Node not found", 404))
				);
			...
			it("should throw error when endpoint config not set", async () => {
				...
				await expect(nodeApi.getNode("test")).rejects.toThrow(
					"Endpoint configuration not set"
				);
			});
		});
	...
	describe("workflowApi", () => {
		...
		describe("getWorkflow", () => {
			it("should fetch a specific workflow by id", async () => {
				...
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchResponse(workflow))
				);
			...
			it("should handle workflow not found", async () => {
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Workflow not found", 404))
				);
		...
		describe("createWorkflow", () => {
			...
			it("should handle validation errors", async () => {
				...
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Validation failed", 400))
				);
		...
		describe("updateWorkflow", () => {
			...
			it("should handle update errors", async () => {
				...
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Update failed", 500))
				);

				await expect(
					workflowApi.updateWorkflow(workflow.id, { name: "New" })
				).rejects.toThrow();
			});
		});
		...
		describe("deleteWorkflow", () => {
			...
			it("should handle delete errors", async () => {
				global.fetch = vi.fn(() =>
					Promise.resolve(mockFetchError("Delete failed", 500))
				);

				await expect(
					workflowApi.deleteWorkflow("test-id")
				).rejects.toThrow();
			});
		});
