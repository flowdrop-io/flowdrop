<script lang="ts">
  import type { ChatHistoryMessage, ChatRequest } from "../../types/chat.js";
  ...
  let messages: ChatHistoryMessage[] = $state([]);
  ...
  const isChatConfigured = $derived(getEndpointConfig()?.endpoints?.chat !== undefined);
  const canSend = $derived(inputValue.trim().length > 0 && !isLoading && !isDisabled && isChatConfigured);
  ...
  async function sendMessage() {
    ...
    try {
      const request: ChatRequest = {
        ...
        history: messages.slice(0, -1), // all except current message (already sent as `message`)
      };
