...
export function createNodeWrapperConfig(
  ...
  return {
    ...
    showStatusDetails:
      config.showDetails ?? DEFAULT_NODE_STATUS_CONFIG.showDetails ?? true,
  };
}
 ...
 */
export function shouldShowNodeStatus(
  executionInfo?: NodeExecutionInfo,
): boolean {
 ...
 */
export function getOptimalStatusPosition(
  nodeType: string,
): NodeStatusConfig["position"] {
  switch (nodeType) {
    case "tool":
      return "top-left";
    case "note":
 ...
 */
export function getOptimalStatusSize(
  nodeType: string,
): NodeStatusConfig["size"] {
  switch (nodeType) {
    case "tool":
      return "sm";
    case "note":
