...
test.describe("Auth Autocomplete Propagation", () => {
  ...
  test("autocomplete includes auth headers and loads suggestions", async ({
    page,
  }) => {
    // Intercept the autocomplete API call to verify auth headers and return mock data
    let capturedAuthHeader: string | null = null;
    await page.route(
      "**/api/flowdrop/autocomplete/auth-users*",
      async (route) => {
    ...
    const autocompleteInput = configPanel
      .locator(".form-autocomplete__input")
      .first();
