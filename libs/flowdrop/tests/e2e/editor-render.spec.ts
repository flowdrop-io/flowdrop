...
test.describe("Editor Rendering", () => {
  ...
  test("canvas renders with nodes and edges from simple workflow", async ({
    page,
  }) => {
  ...
  test("complex workflow renders multiple nodes and edges", async ({
    page,
  }) => {
  ...
  test("sidebar search filters categories", async ({ page }) => {
    ...
    const filteredCategoryCount = await page
      .locator(".flowdrop-details")
      .count();
