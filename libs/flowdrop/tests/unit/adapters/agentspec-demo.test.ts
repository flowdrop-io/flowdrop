...
describe("Demo 1: Agent Spec Node Type Registry", () => {
  it("lists all 9 Agent Spec node types", () => {
    ...
    for (const meta of allTypes) {
      console.log(
        `  ${meta.id.padEnd(28)} | ${meta.name.padEnd(15)} | category: ${meta.category}`,
      );
    }
  ...
  it("shows metadata for an LLM node", () => {
    ...
    console.log(
      `  Inputs:      ${llmMeta!.inputs.map((p) => p.id).join(", ")}`,
    );
    console.log(
      `  Outputs:     ${llmMeta!.outputs.map((p) => p.id).join(", ")}\n`,
    );
  ...
  it("converts a FlowDrop LLM pipeline to Agent Spec format", () => {
    ...
    for (const node of agentSpecFlow.nodes) {
      ...
      if (node.input_properties?.length) {
        console.log(
          `      inputs:  ${node.input_properties.map((p) => p.title).join(", ")}`,
        );
      }
      if (node.output_properties?.length) {
        console.log(
          `      outputs: ${node.output_properties.map((p) => p.title).join(", ")}`,
        );
      }
    }
    console.log(
    ...
    console.log(
      `  Data-flow connections:    ${agentSpecFlow.data_flow_connections?.length ?? 0}`,
    );
    ...
    const agentSpecJson = JSON.stringify(
      {
        ...
        description:
          "Routes customer queries through LLM classification and branching",
        start_node: "intake",
        nodes: [
          {
            name: "intake",
        ...
        control_flow_connections: [
          ...
          {
            from_node: "router",
            to_node: "billing_response",
            from_branch: "billing",
          },
          {
            from_node: "router",
            to_node: "tech_response",
            from_branch: "technical",
          },
          {
            from_node: "router",
            to_node: "general_response",
            from_branch: "general",
          },
        ],
...
describe("Demo 4: Round-trip conversion preserves workflow semantics", () => {
  it("exports then re-imports and compares key properties", () => {
    ...
    const originalTypes = new Set(
      original.nodes.map((n) => n.data.metadata.id),
    );
    const reimportedTypes = new Set(
      reimported.nodes.map((n) => n.data.metadata.id),
    );
...
describe("Demo 5: Full Agent Spec Document export (with agent config)", () => {
  it("wraps a workflow with agent metadata, tools, and LLM config", () => {
    ...
    const json = agentAdapter.exportJSON(
      workflow,
      agentConfig,
      tools,
      llmConfigs,
