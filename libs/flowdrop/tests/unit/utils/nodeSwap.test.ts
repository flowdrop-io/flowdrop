...
describe('computeSwapPreview', () => {
  ...
  describe('Pass 2 — port name match', () => {
    ...
    it('should do case-insensitive name matching', () => {
      ...
      const edges: WorkflowEdge[] = [makeEdge('e1', 'src.1', 'value', 'calculator.1', 'a')];
  ...
  describe('Pass 3 — dataType fallback', () => {
    it('should match by compatible dataType when ID and name differ', () => {
      ...
      const edges: WorkflowEdge[] = [makeEdge('e1', 'src_type.1', 'out1', 'old.1', 'in_z')];
  ...
  describe('data loss scenarios', () => {
    ...
    it('should set hasDataLoss when new node has fewer compatible ports', () => {
      ...
      const preview = computeSwapPreview(advNode, singleInputMeta, edges, [advNode, srcNode], null);
    ...
    it('should drop all input edges when swapping to node with zero inputs', () => {
      ...
      const preview = computeSwapPreview(calcNode, isolatedNode, edges, [calcNode, srcNode], null);
  ...
  describe('handle ID rewriting', () => {
    ...
    it('should rewrite source handles when the swapped node is the source', () => {
      ...
      const edges: WorkflowEdge[] = [makeEdge('e1', 'calculator.1', 'result', 'tgt.1', 'value')];
...
describe('executeSwap', () => {
  ...
  it('should replace old node at the same array index', () => {
    ...
    const preview = computeSwapPreview(calcNode, advancedCalculatorNode, [], allNodes, null);

    const result = executeSwap(calcNode, advancedCalculatorNode, preview, allNodes, []);
  ...
  it('should exclude dropped edges from the result', () => {
    ...
    const result = executeSwap(calcNode, isolatedNode, preview, [calcNode, srcNode], edges);
  ...
  it('should not modify unrelated nodes or edges', () => {
    ...
    const preview = computeSwapPreview(calcNode, advancedCalculatorNode, allEdges, allNodes, null);

    const result = executeSwap(calcNode, advancedCalculatorNode, preview, allNodes, allEdges);

    // Unrelated node untouched
    expect(result.updatedNodes.find((n) => n.id === 'other.1')).toEqual(otherNode);
    // Unrelated edge untouched
    expect(result.updatedEdges.find((e) => e.id === 'e-unrelated')).toEqual(unrelatedEdge);
  });
});
...
describe('mapConfig', () => {
  ...
  it('should carry over matching keys', () => {
    const { config, carriedOver } = mapConfig({ operation: 'multiply' }, schema);
  ...
  it('should discard old-only keys', () => {
    const { config } = mapConfig({ operation: 'add', placeholder: 'hello' }, schema);
  ...
  it('should return empty config for empty schema', () => {
    const { config } = mapConfig({ foo: 'bar' }, { type: 'object', properties: {} });
...
describe('integration — full swap flow', () => {
  ...
  it('should support sequential swaps on different nodes', () => {
    ...
    let allEdges: WorkflowEdge[] = [makeEdge('e1', 'text_input.1', 'value', 'calculator.1', 'a')];

    // Swap node A
    const previewA = computeSwapPreview(nodeA, textFormatterNode, allEdges, allNodes, null);

    const resultA = executeSwap(nodeA, textFormatterNode, previewA, allNodes, allEdges);
    ...
    const resultB = executeSwap(calcNodeAfter!, mathProcessorNode, previewB, allNodes, allEdges);

    // Both swaps should have produced valid node arrays
    expect(resultB.updatedNodes).toHaveLength(2);
    expect(resultB.updatedNodes.find((n) => n.id === 'text_input.1')).toBeUndefined();
    expect(resultB.updatedNodes.find((n) => n.id === 'calculator.1')).toBeUndefined();
  });
  ...
  it('should restore connections when swapping back to original type', () => {
    ...
    const preview1 = computeSwapPreview(calcNode, advancedCalculatorNode, allEdges, allNodes, null);
    const result1 = executeSwap(calcNode, advancedCalculatorNode, preview1, allNodes, allEdges);

    // Both edges kept (ports a and b exist on both)
    expect(result1.updatedEdges).toHaveLength(2);

    // Swap back to calculator
    const advNode = result1.updatedNodes.find((n) => n.id === preview1.newNodeId)!;
...
describe('computeSwapPreviewWithOptions', () => {
  ...
  it('should apply portOverrides to force-drop via null', () => {
    ...
    const preview = computeSwapPreviewWithOptions(
      ...
      [calcNode, srcNode],
      {
        portOverrides: [{ oldPortId: 'a', newPortId: null, direction: 'input' }]
      }
    );
  ...
  it('backwards compatibility — existing computeSwapPreview produces identical results', () => {
    ...
    const oldPreview = computeSwapPreview(calcNode, advancedCalculatorNode, edges, allNodes, null);
...
describe('computeInteractiveState', () => {
  it('should annotate match quality on each port mapping', () => {
    ...
    const state = computeInteractiveState(calcNode, advancedCalculatorNode, edges, [
      calcNode,
      srcNode
    ]);
  ...
  it('should set isFlat=false for object/array config values', () => {
    const nodeWithObjectConfig = makeNode(
      'calculator.1',
      {
        ...calculatorNode,
        configSchema: {
  ...
  it('should provide available new ports', () => {
    ...
    const state = computeInteractiveState(calcNode, advancedCalculatorNode, [], [calcNode]);
...
describe('buildSwapPreviewFromState', () => {
  ...
  it('should preserve connections when port is remapped to different ID (text → message)', () => {
    ...
    const llmAgentNode: NodeMetadata = {
      ...
      outputs: [{ id: 'reply', name: 'Reply', type: 'output', dataType: 'string' }],
      configSchema: { type: 'object', properties: {} },
      tags: []
    };
    ...
    expect(edge1!.targetHandle).toBe(buildHandleId(state.newNodeId, 'input', 'message'));
    ...
    expect(edge2!.sourceHandle).toBe(buildHandleId(state.newNodeId, 'output', 'reply'));
...
describe('performSwap', () => {
  it('should perform a headless one-shot swap', () => {
    ...
    const allNodes = [srcNode, calcNode];
    const allEdges: WorkflowEdge[] = [makeEdge('e1', 'src.1', 'value', 'calculator.1', 'a')];

    const result = performSwap(calcNode, advancedCalculatorNode, allNodes, allEdges);
  ...
  it('should throw SwapValidationError for non-existent node', () => {
    ...
    expect(() => performSwap(calcNode, advancedCalculatorNode, [otherNode], [])).toThrow(
      SwapValidationError
    );
  });
});
...
describe('validateSwapResult', () => {
  ...
  it('should pass for clean result', () => {
    const result = {
      updatedNodes: [makeNode('a.1', textInputNode), makeNode('b.1', textInputNode)],
      updatedEdges: [makeEdge('e1', 'a.1', 'value', 'b.1', 'value')]
    };
  ...
  it('should catch duplicate node IDs', () => {
    const result = {
      updatedNodes: [makeNode('a.1', textInputNode), makeNode('a.1', textInputNode)],
      updatedEdges: []
    };
