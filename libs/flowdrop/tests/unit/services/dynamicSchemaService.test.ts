...
function makeEndpoint(overrides: Partial<DynamicSchemaEndpoint> = {}): DynamicSchemaEndpoint {
	return {
...
describe('dynamicSchemaService', () => {
	...
	describe('fetchDynamicSchema', () => {
		...
		describe('URL resolution', () => {
			...
			it('prepends baseUrl for relative URLs when endpoint config is available', async () => {
				global.fetch = fetchOk(validSchema);
				mockGetEndpointConfig.mockReturnValue({ baseUrl: 'https://api.example.com' });

				const endpoint = makeEndpoint({ url: '/api/schema', parameterMapping: undefined });
	...
	describe('resolveExternalEditUrl', () => {
		...
		it('appends callback URL when callbackUrlParam is configured', () => {
			const linkWithCallback: ExternalEditLink = { ...link, callbackUrlParam: 'return' };
		...
		it('does not append callback URL when callbackUrlParam is absent', () => {
			const url = resolveExternalEditUrl(link, makeNode(), undefined, 'https://app.example.com');
	...
	describe('getEffectiveConfigEditOptions', () => {
		...
		it('returns type config when only metadata.configEdit is set', () => {
			const typeConfig = { dynamicSchema: { url: '/schema', method: 'GET' as const } };
			const node = makeNode({
				data: {
					...makeNode().data,
		...
		it('returns instance config when only extensions.configEdit is set', () => {
			const instanceConfig = { externalEditLink: { url: 'https://admin.example.com' } };
			const node = makeNode({
				data: {
					...makeNode().data,
	...
	describe('clearSchemaCache', () => {
		...
		it('clears only matching entries when called with a pattern', async () => {
			...
			const endpoint1 = makeEndpoint({ url: 'https://api/llm-node/schema', parameterMapping: {} });
		...
		it('returns true when externalEditLink is configured without a dynamic schema', () => {
			const node = makeNode({
				data: {
					...
					metadata: {
						...makeNode().data.metadata,
						configEdit: { externalEditLink: { url: 'https://admin.example.com' } }
					}
				}
			});
