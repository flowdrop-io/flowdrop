 ...
 */
export function createSimpleTemplateAutocomplete(variableHints: string[]): Extension {
	const completionSource = (context: CompletionContext): CompletionResult | null => {
		...
		if (!pathInfo) {
			...
			const beforeCursor = text.slice(Math.max(0, pos - 2), pos);
			if (beforeCursor === "{{") {
				const options: Completion[] = variableHints.map((hint) => ({
					label: hint,
					displayLabel: `⋯ ${hint}`,
					type: "variable"
				}));
		...
		const options: Completion[] = variableHints
