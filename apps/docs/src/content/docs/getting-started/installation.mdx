...
<Aside type="note" title="Multiple Editors Per Page">
  Multiple FlowDrop editors can run on the same page. Each mount gets its own isolated
  `FlowDropInstance` (workflow, history, playground, and panel state). When mounting more than one
  editor with drafts enabled, pass an `instanceId` so their stored drafts don't collide. See the
  [multiple instances guide](/guides/multiple-instances/).
</Aside>
