...
| `mode`           | `'edit' \| 'readonly' \| 'locked'` | `'edit'` | Interaction mode (replaces `readOnly` + `lockWorkflow`); `'readonly'`/`'locked'` disable all canvas editing |
...
| `on*` handlers   | see below                | —       | Flat lifecycle callbacks: `onBeforeSave`, `onAfterSave`, `onSaveError`, `onApiError`, `onWorkflowLoad`, `onBeforeSwap`, `onAfterSwap` (the `eventHandlers` object is the **mount** API only) |
...
| `mode`           | `'edit' \| 'readonly' \| 'locked'` | `'edit'` | Interaction mode (replaces `readOnly` + `lockWorkflow`) |
