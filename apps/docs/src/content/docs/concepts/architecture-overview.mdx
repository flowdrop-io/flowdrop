...
For backward compatibility, the first mount without an explicit `instanceId` becomes the **page-default instance**: it keeps the legacy (unscoped) storage keys and is the instance the module-level store APIs (`getWorkflowStore()`, `workflowActions`, …) operate on. Registries (custom nodes/fields) and settings (theme, port-compatibility config, API endpoint) remain page-global by design. See the [multiple instances guide](/guides/multiple-instances/) for details.
