...
Each mount produces one such tree backed by its own instance; node/field registries and settings are shared across all trees on the page.
...
Every mount creates an isolated `FlowDropInstance` container holding the stores above (workflow, history, playground, interrupts, categories, port coordinates, and pipeline-panel state), resolved through Svelte context. Multiple editors can therefore coexist on one page without sharing state.

The first mount without an explicit `instanceId` becomes the **page-default instance** — what `getInstance()` resolves to for single-editor embeds. In 2.0 there are no module-level store APIs; instances are the API (`getInstance()` inside the tree, the mount handle's `.instance` outside it). Registries (`fd.nodes`/`fd.fields`/`fd.formats`), the API context (`fd.api`), and port compatibility (`fd.portCompatibility`) are now instance-scoped; theme and settings remain page-global by design. See the [multiple instances guide](/guides/multiple-instances/) for details.
...
Both registries are **instance-scoped** — seeded with builtins in the instance constructor and resolved via `getInstance()`. Register after mounting; `BaseRegistry`'s version counter invalidates dependent `$derived` reads so late registrations take effect.
