...
const demoResponses = [
	{
		steps: [
			{ role: 'log', content: 'Loading content from source...', nodeId: 'content_loader.1', metadata: { level: 'info', nodeLabel: 'Content Loader', duration: 320 } },
			{ role: 'log', content: 'Found 12 articles matching criteria', nodeId: 'content_loader.1', metadata: { level: 'info', nodeLabel: 'Content Loader' } },
			{ role: 'log', content: 'Running AI analysis on content...', nodeId: 'ai_content_analyzer.1', metadata: { level: 'info', nodeLabel: 'AI Content Analyzer', duration: 1850 } },
			{ role: 'log', content: 'Analysis complete: 8 items need updates', nodeId: 'ai_content_analyzer.1', metadata: { level: 'info', nodeLabel: 'AI Content Analyzer' } },
			{ role: 'assistant', content: 'I analyzed 12 articles and found **8 items** that need content updates. The AI Content Analyzer identified patterns in naming conventions that should be standardized.\n\nHere\'s a summary:\n- **3 articles** have outdated terminology\n- **5 articles** need formatting improvements\n- All changes have a confidence score above 0.8\n\nWould you like me to apply the suggested changes?' },
		]
	},
	{
		steps: [
			{ role: 'log', content: 'Processing update request...', nodeId: 'simple_agent.1', metadata: { level: 'info', nodeLabel: 'Simple Agent', duration: 450 } },
			{ role: 'log', content: 'Applying content transformations...', nodeId: 'ai_content_analyzer.1', metadata: { level: 'info', nodeLabel: 'AI Content Analyzer', duration: 2100 } },
			{ role: 'log', content: 'Writing results to output...', nodeId: 'text_output.1', metadata: { level: 'info', nodeLabel: 'Text Output', duration: 150 } },
			{ role: 'assistant', content: 'All updates have been applied successfully! Here\'s what was done:\n\n1. **Terminology updates**: Replaced 3 instances of outdated terms\n2. **Formatting fixes**: Standardized heading styles in 5 articles\n3. **Validation**: All changes passed the confidence threshold\n\nThe workflow completed in 2.7 seconds. You can run it again with different parameters if needed.' },
		]
	},
	{
		steps: [
			{ role: 'log', content: 'Starting new analysis cycle...', nodeId: 'content_loader.1', metadata: { level: 'info', nodeLabel: 'Content Loader', duration: 280 } },
			{ role: 'assistant', content: 'I\'ve started a fresh analysis. The Content Loader is scanning for new articles since the last run. I\'ll keep you posted on the results!' },
		]
	}
];
...
export const playgroundHandlers = [
	...
	// Create session
	http.post(`${API_BASE}/workflows/:workflowId/playground/sessions`, async ({ params, request }) => {
